# kolosal-cli

[English version](./README.md)

Super lightweight Ollama + Qwen Code alternative to run Llama 3.3, DeepSeek-R1, Phi-4, Gemma 3, Mistral Small 3.1 and other large language models.

[![x-cmd/install — kolosal-cli Code Quality Monitoring Repo Card](https://x-cmd.com/repo-card/kolosal-cli.svg?lang=zh)](https://x-cmd.com/install/kolosal-cli)

## 安装

```sh
x install kolosal-cli
```

## 代码洞察

合计: **187,425** 行代码（覆盖前 5 种语言、共 **819** 个文件）。

| 语言 | 代码 | 注释 | 空行 | 文件数 |
|------|-----:|-----:|-----:|------:|
| TypeScript | 133,493 | 13,349 | 19,447 | 626 |
| Tsx | 28,025 | 1,972 | 3,522 | 134 |
| Json | 19,865 | 0 | 0 | 16 |
| JavaScript | 4,243 | 614 | 751 | 34 |
| Sh | 1,393 | 238 | 286 | 9 |

## 源代码

- **上游仓库**: <https://github.com/KolosalAI/kolosal-cli>
- **官网**: <https://kolosal.ai>
- **许可证**: Apache-2.0

## 发布

- **最新版本**: `v0.1.3` (2025-10-16)
- **最近提交**: 2025-12-24
- **Release 含资产**: 5 个

## 流行度

- **Star**: 465 · **Fork**: 15 · **开放 issue**: 10 · **贡献者**: 5

## 累计统计

- **发布数**: 5 · **已合并 PR**: 11 · **开放 PR**: 3 · **已关闭 issue**: 0 · **开放 issue**: 10 · **提交数**: 249

## 最近活动

| 时间窗口 | 起始 | 发布 | 已合并 PR | 开放 PR | 已关闭 issue | 开放 issue | 提交 |
|---|---|---:|---:|---:|---:|---:|---:|
| 30d | 2026-08-25 | 0 | 0 | 0 | 0 | 0 | 0 |
| last60d | 2026-07-26 | 0 | 0 | 1 | 0 | 0 | 0 |
| 90d | 2026-06-26 | 0 | 0 | 1 | 0 | 0 | 0 |
| last180d | 2026-03-28 | 0 | 0 | 1 | 0 | 0 | 0 |
| 360d | 2025-09-29 | 4 | 11 | 2 | 0 | 8 | 57 |
| last720d | 2024-10-04 | 5 | 11 | 3 | 0 | 10 | 249 |

## Release 资产

| 资产 | 大小 | 目标平台 |
|------|-----:|----------|
| [kolosal-code-0.1.3-1.x86_64.rpm](https://github.com/KolosalAI/kolosal-cli/releases/download/v0.1.3/kolosal-code-0.1.3-1.x86_64.rpm) | 77.8 MiB | `runtime/rpm/x86_64` |
| [kolosal-code-0.1.3-linux-x64.tar.gz](https://github.com/KolosalAI/kolosal-cli/releases/download/v0.1.3/kolosal-code-0.1.3-linux-x64.tar.gz) | 79.1 MiB | `native/unknown` |
| [kolosal-code_0.1.3_amd64.deb](https://github.com/KolosalAI/kolosal-cli/releases/download/v0.1.3/kolosal-code_0.1.3_amd64.deb) | 53.5 MiB | `runtime/deb/amd64` |
| [KolosalCode-0.1.3-Setup.exe](https://github.com/KolosalAI/kolosal-cli/releases/download/v0.1.3/KolosalCode-0.1.3-Setup.exe) | 182.7 MiB | `other` |
| [KolosalCode-0.1.3-win-x64.zip](https://github.com/KolosalAI/kolosal-cli/releases/download/v0.1.3/KolosalCode-0.1.3-win-x64.zip) | 105.6 MiB | `other` |

## 改进这些数据

kolosal-cli 的安装元数据由 [x-cmd/install](https://github.com/x-cmd/install) 索引维护——这是一份由 x-cmd 在安装时读取的精选 YAML 包列表。如果 `kolosal-cli` 缺失、过期，或安装行为有问题，欢迎在该 repo 提 issue 或 PR：

- **提交 issue**: <https://github.com/x-cmd/install/issues/new>
- **编辑包条目**: <https://github.com/x-cmd/install/edit/main/kolosal-cli.yml>（或索引实际使用的路径）

本页面的数据（card / loc / scorecard / release）由 [x-cmd-install-action](https://github.com/x-cmd-install/x-cmd-install-action) 自动采集，每日重新生成。**安装行为**（版本选择、平台差异、依赖处理）的改进应提交到上游索引。

_数据快照: `data/card/260924.yml` · 2026-09-24T04:46:05Z._
